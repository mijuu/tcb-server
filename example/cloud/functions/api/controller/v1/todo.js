...
class Todo extends BaseContextClass {
  async create () {
    ...
    try {
      const { label } = ctx.request.event.data;
  ...
  async delete () {
    const { ctx, service } = this;
    try {
      const { _id } = ctx.request.event.data;
  ...
  async edit () {
    const { ctx, service } = this;
    try {
      const { _id, label, completed } = ctx.request.event.data;
  ...
  async list() {
    ...
    console.log(this)
    try {
      const { page, size } = ctx.request.event.data;
